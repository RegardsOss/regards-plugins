 ...
 */
package fr.cnes.regards.modules.storage.plugin.smallfiles.utils;
...
import fr.cnes.regards.modules.storage.plugin.smallfiles.task.RetrieveCacheFileTask;
 ...
 **/
public class SmallFilesUtils {

    private static final Logger LOGGER = getLogger(SmallFilesUtils.class);

    private static final int S3_MAX_ATTEMPT = 5;

    private SmallFilesUtils() {
    }
    ...
    /**
     * Remove the suffix {@link fr.cnes.regards.modules.storage.plugin.smallfiles.ISmallFilesStorage#CURRENT_ARCHIVE_SUFFIX} from the
     * given String
     */
    public static String removeSuffix(String name) {
        return name.substring(0, name.length() - ISmallFilesStorage.CURRENT_ARCHIVE_SUFFIX.length());
    }

    /**
     * Remove the prefix {@link fr.cnes.regards.modules.storage.plugin.smallfiles.ISmallFilesStorage#BUILDING_DIRECTORY_PREFIX} from the
     * given String
     */
    public static String removePrefix(String name) {
        if (name.length() > ISmallFilesStorage.BUILDING_DIRECTORY_PREFIX.length()) {
            return name.substring(ISmallFilesStorage.BUILDING_DIRECTORY_PREFIX.length());
        } else {
            return name;
        }
    }
    ...
    public static String computePathOfBuildDirectoryFromArchiveName(String archiveName) {
        return ISmallFilesStorage.BUILDING_DIRECTORY_PREFIX + archiveName.substring(0,
                                                                                    archiveName.indexOf(
                                                                                        ISmallFilesStorage.ARCHIVE_EXTENSION));
    }
    ...
    public static String createArchiveNameFromBuildingDir(String buildingDirectoryName) {
        return SmallFilesUtils.removePrefix(buildingDirectoryName) + ISmallFilesStorage.ARCHIVE_EXTENSION;
    }

    /**
     * Remove the prefix {@link fr.cnes.regards.modules.storage.plugin.smallfiles.ISmallFilesStorage#BUILDING_DIRECTORY_PREFIX} and
     * the suffix {@link fr.cnes.regards.modules.storage.plugin.smallfiles.ISmallFilesStorage#CURRENT_ARCHIVE_SUFFIX} from the
     * given String
     */
    ...
    public static String getLockName(LockTypeEnum lockType,
        ...
        String pathToLock;
        GlacierUrl dispatchedFilePath = dispatchFilePath(node);
        Path zipBuildingRootPath = workspacePath != null ? Path.of(workspacePath, ISmallFilesStorage.ZIP_DIR) : null;
        Path cacheWorkspace = workspacePath != null ? Paths.get(workspacePath, ISmallFilesStorage.TMP_DIR) : null;
        ...
        if (isBuildingDirectory) {
            ...
            if (lockType == LockTypeEnum.LOCK_RESTORE) {
                pathToLock += ISmallFilesStorage.ARCHIVE_EXTENSION;
            }
        ...
        if (rootPath != null) {
            return Paths.get(ISmallFilesStorage.LOCK_PREFIX, "/", rootPath, pathToLock, lockType.toString()).toString();
        } else {
            return Paths.get(ISmallFilesStorage.LOCK_PREFIX, "/", pathToLock, lockType.toString()).toString();
        }
    }

    /**
     * Dispatch an Glacier url into two strings. First one contains stored file path. Second one contains optional
     * smallFile name like url = archive_path?fileName=smallFileName
     */
    public static GlacierUrl dispatchUrl(String url) throws URISyntaxException {
        ...
        Optional<String> fileNameParam = new URIBuilder(url).getQueryParams()
                                                            ...
                                                            .filter(pair -> pair.getName()
                                                                                .equals(ISmallFilesStorage.SMALL_FILE_PARAMETER_NAME))
                                                            .map(NameValuePair::getValue)
                                                            .filter(Objects::nonNull)
                                                            .findFirst();
        return new GlacierUrl(urlWithoutParam, fileNameParam);
    }
     ...
     * smallFile name like url = archive_path?fileName=smallFileName
     */
    public static GlacierUrl dispatchFilePath(String filePath) {
        String[] parts = filePath.split(Pattern.quote("?" + ISmallFilesStorage.SMALL_FILE_PARAMETER_NAME + "="));
        String archiveName = parts[0];
        String smallFileName = null;
        if (parts.length > 1) {
            smallFileName = parts[1];
        }
        return new GlacierUrl(archiveName, Optional.ofNullable(smallFileName));
    }
    ...
    public static String createSmallFilePath(String archivePath, String smallFileName) {
        return String.format("%s?%s=%s", archivePath, ISmallFilesStorage.SMALL_FILE_PARAMETER_NAME, smallFileName);
    }

    public record GlacierUrl(String archiveFilePath,
