...
public class S3ClientReloader<S extends SdkClient> implements AutoCloseable {
    ...
    public <T> Try<T> tryWithClient(CheckedFunction1<S, T> action) {
        return getS3()
                .flatMapTry(s3 -> { // NOSONAR shadowing name is intentional
                    try {
                        T result = action.apply(s3);
                        resetErrors();
                        return Try.success(result);
                    }
                    catch(S3ClientException e) { return Try.failure(e); }
    ...
    public <T> Try<T> logAndWrapInStorageClientException(S s3, Throwable t) {
        LOGGER.error("S3Config {} - {} instance {} - {}",
                config.getKey(), s3.getClass().getSimpleName(), s3.hashCode(), t.getMessage(), t);
    ...
    private synchronized Try<S> getS3() {
        if (s3 != null) { return Try.success(s3); }
        return Try
                .success(config)
                .mapTry(newClient)
    ...
    @Override
    public void close() { Option.of(s3).peek(SdkClient::close); }
}
