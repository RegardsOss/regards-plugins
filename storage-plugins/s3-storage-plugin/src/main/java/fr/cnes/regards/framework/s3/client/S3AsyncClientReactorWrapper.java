...
public class S3AsyncClientReactorWrapper extends S3ClientReloader<S3AsyncClient> implements IS3ClientReactorWrapper {
    ...
    static final ThreadFactory threadFactory = new ThreadFactoryBuilder()
            .setNameFormat("s3-async-client-%d")
            .setUncaughtExceptionHandler((t,e) -> LOGGER.error("Uncaught on thread {}: {}", t.getName(), e.getMessage(), e))
            .build();
    ...
    public Mono<Boolean> exists(String bucket, String key) {
        return withClient(client -> {
            ...
            return fromFutureSupplier(() -> client.headObject(request))
                    .map(any -> {
                        LOGGER.info("Any: {}", any);
                        return true;
                    })
    ...
    public Mono<ResponseAndStream> readContentFlux(String bucket, String key, boolean failIfMissing) {
        return withClient(client -> {
            ...
            return fromFutureSupplier(() -> client.getObject(request, new GetResponseAndStream()))
                    .onErrorMap(SdkClientException.class, S3ClientException::new)
    ...
    public Mono<PutObjectResponse> putContent(String bucket, String path, byte[] content) {
        ...
        return withClient(client -> fromFutureSupplier(() -> client.putObject(putRequest, requestBody)))
            .log("putContent")
            .onErrorMap(SdkClientException.class, S3ClientException::new)
            .onErrorMap(S3Exception.class, this::wrapS3Exception);
    }
    ...
    public Mono<String> initiateMultipartUpload(String bucket, String path) {
        return withClient(client -> {
            ...
            CreateMultipartUploadRequest initRequest = CreateMultipartUploadRequest.builder()
                    ...
                    .key(path)
                    .build();
            return fromFutureSupplier(() -> client.createMultipartUpload(initRequest))
                .map(resp -> resp.uploadId())
                .doOnNext(uploadId -> LOGGER.debug("Multipart {} - Initiated multipart upload to {}/{}", uploadId, bucket, path));
        });
    }
     ...
     * Needed for {@link #completeMultipartUpload(String, String, String, io.vavr.collection.List)}.
     ...
     * {@link #completeMultipartUpload(String, String, String, io.vavr.collection.List)} once you have sent the last part.
     */
    public Mono<UploadedPart> uploadMultipartFilePart(String bucket, String path, String uploadId, int partId, byte[] partBytes) {
        LOGGER.debug("Multipart {} - Uploading part {} of multipart upload to {}/{}...",
                uploadId, partId, bucket, path);
        ...
        return withClient(client -> fromFutureSupplier(() -> client.uploadPart(uploadRequest, requestBody))
            .map(resp -> new UploadedPart(CompletedPart.builder().eTag(resp.eTag()).partNumber(partId).build(), partBytes.length))
            .doOnNext(etag ->
                LOGGER.debug("Multipart {} - Finished uploading part {} of multipart upload to {}/{}, got tag {}",
                    uploadId, partId, bucket, path, etag)));
    }
     ...
     */
    public Mono<String> completeMultipartUpload(String bucket, String path, String uploadId, List<CompletedPart> parts) {
        ...
        CompleteMultipartUploadRequest completeRequest = CompleteMultipartUploadRequest.builder()
            ...
            .multipartUpload(CompletedMultipartUpload.builder()
                .parts(sortedParts.toJavaList())
                .build())
            .build();

        LOGGER.debug("Multipart {} - Completing multipart upload for {}/{}...", uploadId, bucket, path);
        return withClient(client -> fromFutureSupplier(() -> client.completeMultipartUpload(completeRequest))
            .doOnNext(resp -> LOGGER.debug("Multipart {} - Completed multipart upload for {}/{} ", uploadId, bucket, path)))
            .map(CompleteMultipartUploadResponse::eTag);
    }
    ...
    public Mono<String> abortMultipartUpload(String bucket, String path, String uploadId) {
        ...
        LOGGER.debug("Multipart {} - Completing multipart upload for {}/{}...", uploadId, bucket, path);
        return withClient(client -> fromFutureSupplier(() -> client.abortMultipartUpload(abortRequest))
                .doOnNext(resp -> LOGGER.debug("Multipart {} - Aborted multipart upload for {}/{} ", uploadId, bucket, path)))
                .map(any -> uploadId);
    }
    ...
    private static S3AsyncClient createS3Client(StorageConfig config) {
        ...
        return S3AsyncClient.builder()
            ...
            .asyncConfiguration(b ->
                b.advancedOption(SdkAdvancedAsyncClientOption.FUTURE_COMPLETION_EXECUTOR, executor))
            .build();
    }
}
