...
public class FluxByteBufferHarmonizer {
    ...
    public static Function<Flux<ByteBuffer>,Flux<ByteBuffer>> harmonize(int fixedSize) {
        return bbs -> {
            ...
            return bbs
                    ...
                    .concatMap(s -> {
                        switch(s.getType()) {
                            case ON_NEXT: return processNextByteBuffer(fixedSize, fixedSizeBufferRef, s);
                            case ON_COMPLETE: return processComplete(fixedSizeBufferRef.get());
                            case ON_ERROR: return Mono.error(s.getThrowable());
    ...
    public static Publisher<ByteBuffer> processNextByteBuffer(int fixedSize, AtomicReference<ByteBuffer> fixedSizeBufferRef, Signal<ByteBuffer> s) {
