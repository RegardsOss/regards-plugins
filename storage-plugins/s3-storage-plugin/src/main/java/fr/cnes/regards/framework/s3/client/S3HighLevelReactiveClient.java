...
public class S3HighLevelReactiveClient {
    ...
    public Mono<ReadResult> read(Read readCmd) {
        return readingCallableMono(readCmd)
                .subscribeOn(scheduler)
                .onErrorResume(t -> Mono.just(new StorageCommandResult.UnreachableStorage(readCmd, t)))
                .log("storage.s3.read", Level.FINE,
                        SignalType.ON_SUBSCRIBE, SignalType.ON_NEXT, SignalType.ON_ERROR);
    }
    ...
    public Mono<DeleteResult> delete(Delete deleteCmd) {
        return deleteMono(deleteCmd)
                .subscribeOn(scheduler)
                .onErrorResume(t -> Mono.just(new StorageCommandResult.UnreachableStorage(deleteCmd, t)))
                .log("storage.s3.delete", Level.FINE,
                        SignalType.ON_SUBSCRIBE, SignalType.ON_NEXT, SignalType.ON_ERROR);
    }
    ...
    public Mono<CheckResult> check(Check checkCmd) {
        ...
        return getClient(config).exists(bucket, archivePath)
            ...
            .onErrorResume(t -> Mono.just(new StorageCommandResult.UnreachableStorage(checkCmd, t)));
    }
    ...
    protected Function<Mono<WriteResult>, Mono<WriteResult>> dealWithWriteEntryErrors(Write writeCmd) {
        return writeResultMono -> writeResultMono
                .doOnError(t -> LOGGER.error("Command {} - error {}", writeCmd.getCmdId(), t.getMessage(), t))
                .onErrorResume(
                        MultipartException.class,
                        cme -> Mono.just(new WriteFailure(writeCmd)))
                .onErrorResume(t -> Mono.just(new StorageCommandResult.UnreachableStorage(writeCmd, t)))
                .log("storage.s3.write", Level.FINE,
                        SignalType.ON_SUBSCRIBE, SignalType.ON_NEXT, SignalType.ON_ERROR);
    }
    ...
    protected Mono<Long> storeMultipartEntry(Write writeCmd) {
        ...
        return Mono.defer(() -> getClient(config).initiateMultipartUpload(bucket, key)
                .flatMap(uploadId -> uploadThenCompleteMultipartEntry(entry, config, bucket, key, uploadId))
        ).subscribeOn(scheduler);
    }

    protected Mono<Long> uploadThenCompleteMultipartEntry(StorageEntry entry, StorageConfig config, String bucket, String key, String uploadId) {
        return entry.getData()
                ...
                .onErrorResume(t ->
                        getClient(config).abortMultipartUpload(bucket, key, uploadId)
                                .flatMap(any -> Mono.error(new MultipartException(t, entry))));
    }


    private Mono<Long> completeMultipartEntry(StorageConfig config, String bucket, String key, String uploadId, MultipartReport report) {
        return getClient(config).completeMultipartUpload(bucket, key, uploadId, report.getCompleted())
                .map(any -> report.getAccumulatedSize());
    }

    private Publisher<? extends UploadedPart> uploadPart(StorageConfig config, String bucket, String key, String uploadId, Tuple2<ByteBuffer, Integer> part) {
    ...
    protected Mono<Long> storeSmallEntry(Write writeCmd) {
        ...
        return dataToByteArray(entry.getData())
            .flatMap(bytes -> getClient(config)
                .putContent(bucket, key, bytes)
    ...
    private Mono<byte[]> dataToByteArray(Flux<ByteBuffer> data) {
        return data
            ...
            .map(bbs -> bbs.foldLeft(
                new ByteArrayOutputStream(),
                this::appendByteBufferToOutputStream
            )
            .toByteArray());
    }
    ...
    protected Mono<DeleteResult> deleteMono(Delete deleteCmd) {
        return Mono.defer(() -> {
            ...
            return deleteArchiveContent(deleteCmd, bucket, archivePath, client)
                    .transform(deleteErrorManagement(deleteCmd, hasDeleted));
        });
    }
    ...
    private Mono<DeleteResult> deleteArchiveContent(Delete deleteCmd, String bucket, String key, IS3ClientReactorWrapper client) {
        return client.deleteContentWithPrefix(bucket, key)
                .log("storage.s3.delete", Level.FINER,
    ...
    private String getKey(String archivePath, StorageEntry entry) {
        return archivePath + entry.getFullPath();
    }

    public static class MultipartException extends Exception {
        private final StorageEntry entry;
        public MultipartException(Throwable cause, StorageEntry entry) {
            super(cause);
            this.entry = entry;
        }
    }
