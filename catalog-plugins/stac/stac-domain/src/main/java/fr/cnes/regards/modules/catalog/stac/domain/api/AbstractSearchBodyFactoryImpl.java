...
import static fr.cnes.regards.modules.catalog.stac.domain.api.SearchBody.SortBy.Direction.DESC;
...
public abstract class AbstractSearchBodyFactoryImpl {
    ...
    public Try<Fields> parseFields(String repr) {
        return trying(() -> Stream.of(Option.of(repr).getOrElse("").split(","))
                                  .map(String::trim)
                                  .filter(StringUtils::isNotBlank)
                                  .foldLeft(new Fields(new ArrayList<>(), new ArrayList<>()), (f, str) -> {
                                      if (str.startsWith("-")) {
                                          return f.addExclude(str.substring(1));
                                      } else {
                                          return f.addInclude(str);
                                      }
