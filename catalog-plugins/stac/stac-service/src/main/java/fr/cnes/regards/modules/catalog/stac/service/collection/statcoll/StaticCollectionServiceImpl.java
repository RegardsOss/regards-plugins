...
public class StaticCollectionServiceImpl implements StaticCollectionService {
    ...
    @Autowired
    public StaticCollectionServiceImpl(CatalogSearchService catalogSearchService,
    ...
    public List<Tuple2<String, String>> staticRootCollectionsIdsAndLabels() {
        return trying(() -> getRootCollectionsDatasets().map(e -> Tuple.of(e.getIpId().toString(),
                                                                           e.getLabel()))).onFailure(t -> warn(LOGGER,
                                                                                                               "Failed to load the root static collections",
                                                                                                               t))
    ...
    public List<Collection> staticRootCollections(OGCFeatLinkCreator linkCreator, ConfigurationAccessor config) {
        return trying(() -> getRootCollectionsDatasets().flatMap(entity -> convertRequest(entity.getIpId(),
                                                                                          linkCreator,
                                                                                          config))).onFailure(t -> warn(
            LOGGER,
            "Failed to load the root static collections",
    ...
    private Try<Collection> convertRequest(UniformResourceName resourceName,
        ...
        return trying(() -> {
            ...
            return new Collection(StacSpecConstants.Version.STAC_SPEC_VERSION,
                                  ...
                                  null,
                                  null);
        }).mapFailure(COLLECTION_CONSTRUCTION, () -> format("Failed to build collection for URN %s", resourceName));
    }

    private List<Link> getLinks(UniformResourceName resourceName, OGCFeatLinkCreator linkCreator, String urn)
        throws SearchException, OpenSearchUnknownParameter {
        final List<Link> links;

        if (resourceName.getEntityType().equals(EntityType.COLLECTION)) {
            List<AbstractEntity<?>> children = getSubCollectionsOrDatasets(urn, EntityType.COLLECTION).appendAll(
                getSubCollectionsOrDatasets(urn, EntityType.DATASET));
    ...
    private Option<Link> getItemsLinks(UniformResourceName resourceName, OGCFeatLinkCreator linkCreator) {
        return linkCreator.createCollectionItemsLinkWithRel(idMappingService.getStacIdByUrn(resourceName.toString()),
                                                            ITEMS);
    }
    ...
    private List<Link> getParentCollectionId(String urn, OGCFeatLinkCreator linkCreator)
        ...
        return parentCollectionsId.map(x -> linkCreator.createCollectionLinkWithRel(x, "", "parent")).flatMap(t -> t);
    }
    ...
    private List<String> getParentCollectionsId(String urn) throws SearchException, OpenSearchUnknownParameter {
        ...
        return searchCriterion(tags, EntityType.COLLECTION).map(AbstractEntity::getIpId)
                                                           .map(Object::toString)
