...
public class CollectionServiceImpl implements CollectionService, StacLinkCreator {
    ...
    public Collection buildRootDynamicCollection(OGCFeatLinkCreator linkCreator, ConfigurationAccessor config) {
        ...
        return new Collection(StacSpecConstants.Version.STAC_SPEC_VERSION, List.empty(), name, DEFAULT_DYNAMIC_ID, name,
                dynamicCollectionLinks(linkCreator, DEFAULT_DYNAMIC_ID, name, new DynCollVal(def, List.empty())),
                List.empty(), "", List.empty(), Extent.maximalExtent(), // no extent at this level
    ...
    public Try<CollectionsResponse> buildRootCollectionsResponse(OGCFeatLinkCreator linkCreator,
        ...
        return trying(() -> new CollectionsResponse(buildCollectionsLinks(linkCreator),
                buildRootCollections(linkCreator, config))).mapFailure(COLLECTIONSRESPONSE_CONSTRUCTION,
    ...
    @Override
    public Try<ItemCollectionResponse> getItemsForCollection(String collectionId, Integer limit, Integer page, BBox bbox,
            String datetime, OGCFeatLinkCreator ogcFeatLinkCreator,
        ...
        return tryIsb.flatMap(isb -> itemSearchService.search(isb, page, ogcFeatLinkCreator,
