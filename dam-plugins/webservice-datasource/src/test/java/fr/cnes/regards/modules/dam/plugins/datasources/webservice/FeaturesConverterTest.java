...
public class FeaturesConverterTest extends AbstractRegardsServiceTransactionalIT {
    ...
    private static AttributeModel mockAttributeModel(String path, PropertyType type, boolean optional) {
    ...
    private static ModelAttrAssoc mockModelAttrAssoc(String path, PropertyType type, boolean optional) {
    ...
    public void testSimpleConversion() throws IOException, ModuleException, DataSourceException {
        ...
        Pair<List<DataObjectFeature>, Integer> converted = converter.getConvertedResults();
        Assert.assertEquals("Page size should have been correctly retrieved", 50, converted.getSecond().intValue());
        List<DataObjectFeature> convertedFeatures = converted.getFirst();
        Assert.assertEquals("2 features should have been converted", 2, convertedFeatures.size());
        // Check feature 1
        Assert.assertEquals(convertedFeatures.get(0).getLabel(), "Feature 1");
        Assert.assertEquals(convertedFeatures.get(0).getProviderId(), "ProviderId1");
        // Check feature 2
        Assert.assertEquals(convertedFeatures.get(1).getLabel(), "Feature 2");
        Assert.assertEquals(convertedFeatures.get(1).getProviderId(), "ProviderId2");
    }
    ...
    public void testFullConversion() throws IOException, ModuleException, DataSourceException {
        ...
        Assert.assertEquals(50, convertedResults.getSecond().intValue());
        List<DataObjectFeature> convertedFeatures = convertedResults.getFirst();
        Assert.assertEquals("There should be 2 converted features", 2, convertedFeatures.size());

        // First feature
        DataObjectFeature feat1 = convertedFeatures.get(0);
        ...
        Optional<IProperty<?>> f1AttrA = f1.getValue().stream().filter(p -> p.getName().equals("attrA")).findFirst();
        ...
        Optional<IProperty<?>> f1AttrB = f1.getValue().stream().filter(p -> p.getName().equals("attrB")).findFirst();
        ...
        // Second feature
        DataObjectFeature feat2 = convertedFeatures.get(1);
        ...
        IProperty<?> attrC = feat2.getProperty("attrC");
    ...
    public void testTHEIAConversion() throws IOException, ModuleException, DataSourceException {
        ...
        List<DataObjectFeature> convertedResults = converter.getConvertedResults().getFirst();
        Assert.assertEquals("Page size should be correctly converted", 20, convertedResults.size());
        Assert.assertEquals("All elements should have been converted", 20, convertedResults.size());

        // Check each feature has the right attributes set
        for (DataObjectFeature feat : convertedResults) {
            ...
            for (String attrPath : attrMap.keySet()) {
                if (attrPath.equals(StaticProperties.FEATURE_LABEL)) {
                    ...
                    Assert.assertNotNull("Provider ID should not be null in feature " + feat.getLabel(),
                                         feat.getProviderId());
                } else {
