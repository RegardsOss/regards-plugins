...
public class FeaturesConverter {
     ...
     */
    private static final List<DataType> imagesDataTypes = Arrays.asList(DataType.QUICKLOOK_HD,
     ...
     */
    private static final List<String> imagesAllowedMimeTypes = Arrays.asList(MediaType.IMAGE_GIF_VALUE,
            ...
            if ((modelAttrAssocs != null) && !modelAttrAssocs.isEmpty()) {
                for (Map.Entry<String, String> attributeToPath : attributesToPathCopy.entrySet()) {
                    ...
                    Optional<ModelAttrAssoc> optionalAttribute = modelAttrAssocs.stream()
                                                                                .filter(modelAttrAssoc -> modelAttrAssoc.getAttribute()
                                                                                                                        .getJsonPath()
                                                                                                                        .equals(
                    ...
                    if (optionalAttribute.isPresent()) {
                        ...
                        throw new ModuleException(String.format(
                            "Webservice data source plugin: cannot retrieve attribute model %s. Please check plugin configuration",
                            attributeToPath.getKey()));
                    }
                }
            }
    ...
    private DataObjectFeature convertFeature(String tenant, Feature feature, int featureIndex) {
        ...
        String labelValue = getStringAttributeValueByPath(feature,
                                                          labelJSONPath,
                                                          featureIndex,
                                                          null,
                                                          null,
        ...
        String providerIDValue = getStringAttributeValueByPath(feature,
                                                               providerIdJSONPath,
                                                               featureIndex,
                                                               labelValue,
                                                               null,
        ...
        DataObjectFeature convertedFeature = new DataObjectFeature(OaisUniformResourceName.pseudoRandomUrn(
            OAISIdentifier.AIP,
            EntityType.DATA,
            tenant,
            1), providerIDValue, labelValue);
     ...
     */
    private void convertAttribute(String sourceJSonPath,
                                  Feature sourceFeature,
                                  AttributeModel targetAttributeModel,
                                  DataObjectFeature targetFeature,
                                  String label,
                                  String providerId,
                                  int featureIndex) {
        ...
        try {
            convertedAttribute = IProperty.forType(targetAttributeModel.getType(),
                                                   targetAttributeModel.getName(),
                                                   sourceValue);
        } catch (IllegalArgumentException | ClassCastException e) {
            // Conversion failed: build attribute with null value
            LOGGER.warn(String.format(
                "Webservice data source plugin: %s attribute value, of type %s, cannot be converted from %s (%s) in feature #%d %s(%s)",
                targetAttributeModel.getJsonPath(),
                targetAttributeModel.getType(),
                sourceJSonPath,
                sourceValue,
            ...
            report.addFeatureConversionError(featureIndex,
                                             label,
                                             providerId,
                                             FeatureConversionError.getValueNotConvertibleError(targetAttributeModel.getJsonPath(),
                                                                                                targetAttributeModel.getType(),
                                                                                                !targetAttributeModel.isOptional(),
                                                                                                sourceJSonPath,
                                                                                                sourceValue));
     ...
     * @param featureIndex      index in page, for reporting
     */
    private void convertFile(String fileURLSourcePath,
                             Feature sourceFeature,
                             DataObjectFeature targetFeature,
                             DataType fileType,
                             String label,
        ...
        String fileURL = getStringAttributeValueByPath(sourceFeature,
                                                       fileURLSourcePath,
                                                       featureIndex,
                                                       label,
                                                       providerId,
        ...
        try {
            ...
            LOGGER.warn(String.format(
                "Webservice data source plugin: Found invalid URL '%s' at path '%s' while converting feature #%d-%s(%s) %s",
                fileURL,
                fileURLSourcePath,
                featureIndex,
                label,
            ...
            report.addFeatureConversionError(featureIndex,
                                             label,
                                             providerId,
                                             FeatureConversionError.getInvalidFileURLError(fileURL,
                                                                                           fileType,
                                                                                           fileURLSourcePath));
            return; // Invalid URL
        }
        ...
        if (imagesDataTypes.contains(fileType)) {
            ...
            if (inferredMimeType == null) {
                ...
                report.addFeatureConversionError(featureIndex,
                                                 label,
                                                 providerId,
                                                 FeatureConversionError.getMimeTypeNotFoundError(fileURL,
                                                                                                 fileType,
                                                                                                 fileURLSourcePath));
                return;
                ...
                report.addFeatureConversionError(featureIndex,
                                                 label,
                                                 providerId,
                                                 FeatureConversionError.getInvalidImageMimeTypeFound(fileURL,
                                                                                                     fileType,
                                                                                                     fileURLSourcePath,
                                                                                                     inferredMimeType));
                return;
     ...
     */
    private String getStringAttributeValueByPath(Feature sourceFeature,
                                                 String path,
                                                 int featureIndex,
                                                 String label,
                                                 String providerId,
                                                 ...
                                                 boolean mandatory) {
        Object valueByPath = getAttributeValueByPath(sourceFeature,
                                                     path,
                                                     featureIndex,
                                                     label,
                                                     providerId,
     ...
     * @return found value or null
     */
    private Object getAttributeValueByPath(Feature sourceFeature,
                                           String path,
                                           int featureIndex,
                                           String label,
                                           String providerId,
            ...
            if (i == 0) {
                ...
                if (currentValue instanceof Map) {
                    ...
                    report.addFeatureConversionError(featureIndex,
                                                     label,
                                                     providerId,
                                                     FeatureConversionError.getInvalidValueOnPathError(
                                                         targetAttributePath,
                                                         mandatory,
                                                         path,
                                                         pathElements.get(i - 1),
        ...
        if (mandatory && (currentValue == null)) {
            report.addFeatureConversionError(featureIndex,
                                             label,
                                             providerId,
                                             FeatureConversionError.getMandatoryAttributeNotFoundError(
                                                 targetAttributePath,
                                                 path));
        }
