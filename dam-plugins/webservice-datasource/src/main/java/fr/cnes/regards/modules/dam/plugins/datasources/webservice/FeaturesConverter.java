...
public class FeaturesConverter {
     ...
     */
    public FeaturesConverter(ConversionConfiguration conversionConfiguration, List<ModelAttrAssoc> modelAttrAssocs) throws ModuleException {
        ...
        if (!attributesToPathCopy.isEmpty()) {
            if (modelAttrAssocs != null && !modelAttrAssocs.isEmpty()) {
                for (Map.Entry<String, String> attributeToPath : attributesToPathCopy.entrySet()) {
                    ...
                    Optional<ModelAttrAssoc> optionalAttribute = modelAttrAssocs.stream().filter(modelAttrAssoc ->
                            modelAttrAssoc.getAttribute().getJsonPath().equals(attributeToPath.getKey())).findFirst();
                    ...
                    if (optionalAttribute.isPresent()) {
                        ...
                        throw new ModuleException(
                                String.format("Webservice data source plugin: cannot retrieve attribute model %s. Please check plugin configuration", attributeToPath.getKey()));
                    }
                }
            }
        }
     ...
     */
    private static int retrieveMandatoryIntField(FeatureWithPropertiesCollection featureCollection, String fieldName) throws DataSourceException {
        Object foundValue = featureCollection.getProperties().get(fieldName);
        if (foundValue == null) {
            throw new DataSourceException(String.format("The field '%s' is missing in results page properties", fieldName));
        }
        ...
        try {
            ...
            throw new DataSourceException(String.format("Field '%s' value cannot be parsed into a valid integer number", fieldName), e);
        }
    }
     ...
     */
    public void convert(String tenant, Pageable page, FeatureWithPropertiesCollection featureCollection) throws DataSourceException {
    ...
    private DataObjectFeature convertFeature(String tenant, Feature feature, int featureIndex) {
        ...
        String labelValue = getStringAttributeValueByPath(feature, labelJSONPath, featureIndex, null, null, StaticProperties.FEATURE_LABEL, true);

        // B - Extract mandatory attribute provider ID
        LOGGER.trace("Webservice data source plugin: Retrieve feature provider ID");
        String providerIDValue = getStringAttributeValueByPath(feature, providerIdJSONPath, featureIndex, labelValue, null, StaticProperties.FEATURE_PROVIDER_ID, true);
        ...
        LOGGER.trace(String.format("Webservice data source plugin: convert other attributes in feature %s(%s)", labelValue, providerIDValue));
        attributeModelToPath.forEach((key, value) -> this.convertAttribute(value, feature, key, convertedFeature, labelValue, providerIDValue, featureIndex));

        // F - convert files
        String quicklookURLPath = conversionConfiguration.getQuicklookURLPath();
        if (!Strings.isNullOrEmpty(quicklookURLPath)) {
            convertFile(quicklookURLPath, feature, convertedFeature, DataType.QUICKLOOK_SD, labelValue, providerIDValue, featureIndex);
        }
        String rawDataURLPath = conversionConfiguration.getRawDataURLPath();
        if (!Strings.isNullOrEmpty(rawDataURLPath)) {
            convertFile(rawDataURLPath, feature, convertedFeature, DataType.RAWDATA, labelValue, providerIDValue, featureIndex);
        }
        String thumbnailURLPath = conversionConfiguration.getThumbnailURLPath();
        if (!Strings.isNullOrEmpty(thumbnailURLPath)) {
            convertFile(thumbnailURLPath, feature, convertedFeature, DataType.THUMBNAIL, labelValue, providerIDValue, featureIndex);
        }
    ...
    private void convertAttribute(String sourceJSonPath, Feature sourceFeature, AttributeModel targetAttributeModel,
        ...
        LOGGER.trace(String.format("Webservice data source plugin: convert attribute from %s to %s(%s)",
                sourceJSonPath, targetAttributeModel.getJsonPath(), targetAttributeModel.getType()));

        // A - Retrieve source value (let delegate method add mandatory and path errors if any)
        Object sourceValue = this.getAttributeValueByPath(sourceFeature, sourceJSonPath, featureIndex, label, providerId, targetAttributeModel.getJsonPath(), !targetAttributeModel.isOptional());

        // B - Attempt value conversion (if it fails, initialize attribute with null but report the error)
        AbstractAttribute<?> convertedAttribute;
        try {
            convertedAttribute = AttributeBuilder.forType(targetAttributeModel.getType(), targetAttributeModel.getName(), sourceValue);
        } catch (IllegalArgumentException | ClassCastException e) {
            // Conversion failed: build attribute with null value
            LOGGER.warn(String.format("Webservice data source plugin: %s attribute value, of type %s, cannot be converted from %s (%s) in feature #%d %s(%s)",
                    targetAttributeModel.getJsonPath(), targetAttributeModel.getType(), sourceJSonPath, sourceValue, featureIndex, label, providerId), e);
            report.addFeatureConversionError(featureIndex, label, providerId, FeatureConversionError.getValueNotConvertibleError(
                    targetAttributeModel.getJsonPath(), targetAttributeModel.getType(), !targetAttributeModel.isOptional(), sourceJSonPath, sourceValue));
            convertedAttribute = AttributeBuilder.forType(targetAttributeModel.getType(), targetAttributeModel.getName(), null);
        }
     ...
     * @param featureIndex      index in page, for reporting
     */
    private void convertFile(String fileURLSourcePath, Feature sourceFeature, DataObjectFeature targetFeature, DataType fileType, String label, String providerId, int featureIndex) {
        // extract file URL as optional value
        String fileURL = getStringAttributeValueByPath(sourceFeature, fileURLSourcePath, featureIndex, label, providerId, fileType.toString(), false);
        if (fileURL == null) {
            return; // NO URL
        }
        try {
            ...
            LOGGER.warn(String.format("Webservice data source plugin: Found invalid URL '%s' at path '%s' while converting feature #%d-%s(%s) %s",
                    fileURL, fileURLSourcePath, featureIndex, label, providerId, fileType), e);
            report.addFeatureConversionError(featureIndex, label, providerId, FeatureConversionError.getInvalidFileURLError(fileURL, fileType, fileURLSourcePath));
            return; // Invalid URL
        }
        ...
        if (imagesDataTypes.contains(fileType)) {
            ...
            if (inferredMimeType == null) {
                ...
                LOGGER.warn(String.format("Webservice data source plugin: Could not guess mime type from URL '%s' at path '%s' while converting feature #%d-%s(%s) %s",
                        fileURL, fileURLSourcePath, featureIndex, label, providerId, fileType));
                report.addFeatureConversionError(featureIndex, label, providerId, FeatureConversionError.getMimeTypeNotFoundError(fileURL, fileType, fileURLSourcePath));
                return;
            } else if (!imagesAllowedMimeTypes.contains(inferredMimeType)) {
                LOGGER.warn(String.format("Webservice data source plugin: Found a not supported MIME type '%s' for URL '%s' at path '%s' while converting feature #%d-%s(%s) %s",
                        inferredMimeType, fileURL, fileURLSourcePath, featureIndex, label, providerId, fileType));
                report.addFeatureConversionError(featureIndex, label, providerId, FeatureConversionError.getInvalidImageMimeTypeFound(fileURL, fileType, fileURLSourcePath, inferredMimeType));
                return;
            }
        ...
        targetFeature.getFiles().put(fileType, DataFile.build(fileType, filename, fileURL, mediaType, Boolean.TRUE, Boolean.TRUE));
    }
     ...
     */
    private String getStringAttributeValueByPath(Feature sourceFeature, String path, int featureIndex, String label, String providerId, String targetAttributePath, boolean mandatory) {
        Object valueByPath = getAttributeValueByPath(sourceFeature, path, featureIndex, label, providerId, targetAttributePath, mandatory);
    ...
    private Object getAttributeValueByPath(Feature sourceFeature, String path, int featureIndex, String label,
        ...
        for (int i = 0; i < pathElements.size(); i++) {
            ...
            if (i == 0) {
                ...
                if (currentValue instanceof Map) {
                    ...
                    LOGGER.warn(String.format("When converting feature at %d, attribute %s value could not be extracted from path '%s' as value at '%s' (%s) is not a Json object.",
                            featureIndex, targetAttributePath, path, pathElements.get(i - 1), currentValue.toString()));
                    report.addFeatureConversionError(featureIndex, label, providerId,
                            FeatureConversionError.getInvalidValueOnPathError(targetAttributePath, mandatory, path, pathElements.get(i - 1), currentValue));
                    return null;
                }
            }
        }
        if (mandatory && currentValue == null) {
            report.addFeatureConversionError(featureIndex, label, providerId,
                    FeatureConversionError.getMandatoryAttributeNotFoundError(targetAttributePath, path));
        }
