...
public class SimpleShellProcessOneToOnePluginTest {
    ...
    public void runSimpleScript() throws Exception {
        ...
        IWorkloadEngine engine = SimpleProcessPluginTestUtils.makeEngine();
        IWorkloadEngineRepository engineRepo = SimpleProcessPluginTestUtils.makeEngineRepo(engine);
        OrderProcessRepositoryImpl processRepo = SimpleProcessPluginTestUtils.makeProcessRepo(engineRepo);
        SimpleShellProcessPlugin shellProcessPlugin = makePlugin(workdirService, storageService);

        RightsPluginConfiguration rpc = SimpleProcessPluginTestUtils.makeRightsPluginConfig(SimpleShellProcessPlugin.SIMPLE_SHELL_PROCESS_PLUGIN);
        PProcess process = Objects.requireNonNull(processRepo.fromPlugin(rpc, shellProcessPlugin, "tenant").block());
        PBatch batch = SimpleProcessPluginTestUtils.makeBatch(batchId, process);
        PExecution exec = SimpleProcessPluginTestUtils.makeExec(execId, batchId, batch.getProcessBusinessId());
