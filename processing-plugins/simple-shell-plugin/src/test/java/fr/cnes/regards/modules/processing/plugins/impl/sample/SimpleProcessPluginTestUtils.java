...
public class SimpleProcessPluginTestUtils {
    ...
    public static PExecution makeExec(UUID execId, UUID batchId, UUID processBusinessId) throws Exception {
        return new PExecution(execId,
                              "exec cid",
                              batchId,
                              "order-1_dsSel-2_subOrder-4",
                              Duration.ofSeconds(10),
                              List.of(new PInputFile("one",
                                                     "one.raw",
                                                     "text/plain",
                                                     resourceFileBasePath.resolve("one.raw").toUri().toURL(),
                                                     3L,
                                                     "checksum",
                                                     "one.raw",
                                                     inputMetadataAsMap(false, "urn"),
                                                     "one"),
                                      new PInputFile("two",
                                                     "two.raw",
                                                     "text/plain",
                                                     resourceFileBasePath.resolve("two.raw").toUri().toURL(),
                                                     3L,
                                                     "checksum",
                                                     "two.raw",
                                                     inputMetadataAsMap(false, "urn"),
    ...
    public static OrderProcessRepositoryImpl makeProcessRepo(IWorkloadEngineRepository engineRepo) {
       ...
       /* when(authFactory.authFromUserEmailAndRole(anyString(), anyString(), anyString()))
        ...
        return new OrderProcessRepositoryImpl(mock(IPluginService.class),
                                              engineRepo,
                                              rightsRepo,
    ...
    public static OrderProcessRepositoryImpl makeProcessRepo(IWorkloadEngineRepository engineRepo,
        ...
        IPUserAuthService authFactory = mock(IPUserAuthService.class);
        when(authFactory.authFromUserEmailAndRole(anyString(), anyString(), anyString())).thenAnswer(i -> new PUserAuth(
            i.getArgument(0),
            i.getArgument(1),
            i.getArgument(2),
            "authToken"));
